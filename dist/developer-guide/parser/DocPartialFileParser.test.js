...
const DocPartialFileParser_1 = require("./DocPartialFileParser");
/** */
describe('DocSourceParser', () => {
    const expectPartial = (partial, id, content) => {
    ...
    `;
        const parser = new DocPartialFileParser_1.DocPartialFileParser();
        const partials = parser.parse('file.ts', content);
        expect(partials).toHaveLength(1);
        expectPartial(partials[0], id, 'This is a test partial.\n\nIt ends here.');
    });
    it('should parse a TypeScript file with a partial at the end ', () => {
      ...
      // That's all folks.`;
        ...
        const partials = parser.parse('file.ts', content);
        expect(partials).toHaveLength(1);
        expectPartial(partials[0], id, `That's all folks.`);
    });
    ...
    `;
        ...
        expectPartial(partials[0], id0, 'This is a test partial.\n\nIt ends here.');
        expectPartial(partials[1], id1, 'This is another test partial.');
    });
